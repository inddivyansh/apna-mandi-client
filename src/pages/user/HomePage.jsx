...
export default function HomePage({ setCurrentPage }) {
  const { user } = useStore();
  const WHATSAPP_NUMBER = "911234567890"; // Replace with a real number
  const WHATSAPP_MESSAGE = "I need help with my Apna Mandi account.";
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border">
        ...
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => setCurrentPage('history')} className="flex flex-col items-center justify-center p-4 bg-gray-100 rounded-lg hover:bg-orange-100 transition-colors">
           ...
           <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(WHATSAPP_MESSAGE)}`} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center justify-center p-4 bg-gray-100 rounded-lg hover:bg-green-100 transition-colors">
